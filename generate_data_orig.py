...
def get_obj_files(shapenet_path, cc_textures, type='shapenet', num_objects=1, categories_file = None):
    """
    Load objects from ShapeNet or BOP dataset, apply random textures, and set physics properties.
    Parameters:
    shapenet_path (str): Path to the ShapeNet dataset.
...
def generate_scene(
    ...
    bproc.camera.set_resolution(int(640*0.5), int(480*0.5))
    ...
    if include_object:
        # Load objects
        objs, cc_textures,objs_ch = get_objs(shapenet_path, cc_textures, type=dataset_type, num_objects=num_samples,categories_file='categories.txt')
        ...
        for obj, obj_ch in zip(objs, objs_ch):
            ...
            if obj_width>1.5*np.linalg.norm(l_finger_pos - r_finger_pos):
                ...
                while CollisionUtility.check_mesh_intersection(joint_obj,obj) and CollisionUtility.is_point_inside_object(point=joint_obj.get_location(),obj_bvh_tree=link_bvh_tree,obj=obj) and  iter<1/iter*1.5:
                    ...
                    obj_ch.set_location(obj_ch.get_location() + joint_middle_vec*step_move)
                    iter+=1
                ...
                rotations = {32:0, 38:0}
                bvh_tree = obj_ch.create_bvh_tree()
                patience = 10
                patience_iter = {32:0, 38:0}
                # We progressively move each finger to check for collisions with the object            
                while rotations[32] < 1.1*.8/.005 and rotations[38] and not (has_collided[32] and has_collided[38]):
                    joint_positions = robot.get_all_local2world_mats()
                    # middle of fingers
                    l_finger_pos = joint_positions[35][:3, 3]
                    r_finger_pos = joint_positions[41][:3, 3]
                    ...
                    for finger_idx,finger_pos in finger_positions.items():
                        ...
                        if collision:
                            print(f'Collision detected with finger {finger_idx} at iteration {rotations}, patience {patience_iter[finger_idx]}')
                            patience_iter[finger_idx]+=1
                            if patience_iter[finger_idx]>patience:
                                has_collided[finger_idx] = True
                                continue
                        elif patience_iter[finger_idx]>0:
                ...
                if rotations[32]<10 and rotations[38]<10:
                    robot.set_rotation_euler_fk(link=robot.links[32], rotation_euler=-0.005*rotations[38], mode='absolute')
                if rotations[38]<10 and rotations[32]<10:
                    robot.set_rotation_euler_fk(link=robot.links[38], rotation_euler=-0.005*rotations[32], mode='absolute')
                obj_ch.hide(True)
