...
def get_obj_filenames(shapenet_path, cc_textures, type='shapenet', categories_file = None):
    """
...
def get_distractor_objects(obj_files, cc_textures, num_objects = 1):
    ...
    for obj_file in np.random.choice(obj_files, num_objects, replace=False):
        ...
        target_size = np.random.uniform(0.1, 0.5)
...
def close_grippers(obj, obj_cv, robot):
    ...
    logging.info(f'Rotated {rotations} times, collision: {has_collided}')
...
def generate_scene(
    ...
    # Generate scenes
    obj_files = get_obj_filenames(shapenet_path, cc_textures, type=dataset_type, categories_file='categories.txt')
    for scene in range(num_scenes):
        
        # We get the distractor objects
        distractor_objs = get_distractor_objects(obj_files, cc_textures, num_objects=np.random.randint(5,15))
...
if __name__ == "__main__":
    ...
    print(config)
    print("---"*10)
    logging.info(f'Started Rendering script:\n - number of samples {args.num_scenes}, {args.num_per_scene} and ratio {args.positive_ratio}\nOther args: '+str(config))
    bproc.init()
